...
<table class="table table-bordered table-hover pure-table" id="permissions_table">
                        ...
                        <td>
                            <label class="switch">
                                <input class="permission-button" data-id="<?php echo $permission->id; ?>"
                                    type="checkbox"
                                    <?php if ($permission->role_permission_count > 0) {
                                                                                                                            echo "checked";
                                                                                                                        } ?>>
...
<script>
$(document).on('change', '.permission-button', function() {
    var id = $(this).attr('data-id');
    $.post("{{route('permission.update')}}", '_token={{ csrf_token() }}&role_id=<?= $id; ?>&permission_id=' +
        id,
