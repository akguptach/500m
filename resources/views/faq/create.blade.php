                ...
                <div class="col-md-12">
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Create <small>FAQ</small></h3>
                        </div>
                        <form id="quickForm" method="POST" action="{{route('faq.store')}}">
                            ...
                            <div class="card-body">
                                    ...
                                    <select name="website_id" class="form-control">
                                        ...
                                        @if(!empty($websites))
                                          @foreach($websites as $website1)
                                            <option value="{{$website1->id}}"  @if(old("website_id") == $website1->id) selected @endif>{{$website1->website_name }}</option>
                                          @endforeach
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    <label >Answer</label>
                                    <input type="text" name="answer" class="form-control"placeholder="Enter Answer" value="{{old('answer')}}">
