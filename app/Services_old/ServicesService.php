    ...
    public function getPages($type)
    {
        ...
        if (!empty($reqRecord['data'])) {
            foreach ($reqRecord['data'] as $index => $item) {
                ...
                $reqRecord['data'][$index]['action'] = "<a class='btn btn-xs sharp btn-primary' href='" . url($editItem) . "' >
                <i class='fas fa-edit' title='Edit'></i></a>
                &nbsp;&nbsp;&nbsp;
                <a class='btn btn-xs sharp btn-danger' href='javascript:void(0);' onclick='delete_service(" . $del_msg . "," . $req_serv_id . ")' >
